...
public class PlaybackService extends Service {
    ...
    private File currentFile, pendingFile;
    ...
    private void prepare(File file, String name) {
        try {
            pending = new MediaPlayer(); pendingFile = file;
            ...
            pending.setDataSource(file.getAbsolutePath());
            pending.setOnPreparedListener(mp -> {
                ...
                for (MediaPlayer.TrackInfo track : mp.getTrackInfo()) {
                    hasAudio |= track.getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_AUDIO;
                    hasVideo |= track.getTrackType() == MediaPlayer.TrackInfo.MEDIA_TRACK_TYPE_VIDEO;
                }
                if (!hasAudio || hasVideo) { cancelPending(); report("请选择纯音频文件，不支持视频。"); return; }
                if (player != null) player.release();
                if (currentFile != null) currentFile.delete();
                abandonFocus(); player = mp; currentFile = file; pending = null; pendingFile = null;
                title = name; ready = true; busy = false; ended = false; error = ""; applyVolume(); refreshOutputs();
                player.setOnCompletionListener(p -> { ended = true; abandonFocus(); refreshNotification(); update(); });
        ...
        } catch (Exception e) { cancelPending(); file.delete(); report("无法加载音频：" + e.getMessage()); }
    }
    ...
    void update() {
        ...
        String status = busy ? "正在导入 / 读取音频…" : playing ? "正在播放 · 本地音频" : ready ? "已就绪 · 点击播放" : "选择一首本地音频，开始试听";
        try {
        ...
        if (session != null) session.setPlaybackState(new PlaybackState.Builder().setActions(PlaybackState.ACTION_PLAY | PlaybackState.ACTION_PAUSE | PlaybackState.ACTION_PLAY_PAUSE | PlaybackState.ACTION_SEEK_TO | PlaybackState.ACTION_STOP)
    ...
    private void showOverlay() {
        ...
        panel = new TextView(this); panel.setText("♪"); panel.setTextSize(30); panel.setTextColor(Color.rgb(24,44,39)); panel.setGravity(Gravity.CENTER);
        ...
        panel.setOnClickListener(v -> {
            try { startActivity(new Intent(this, PlayerActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_SINGLE_TOP)); }
            catch (Exception e) { report("请从通知打开音乐界面。"); }
        });
    ...
    private void clampWindow() {
        ...
        windowParams.y=Math.max(0,Math.min(windowParams.y,height-dp(120)));
    }
    ...
    @Override public void onDestroy() {
        ...
        cancelPending(); if(currentFile != null) currentFile.delete(); abandonFocus();
