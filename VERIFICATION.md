...
## Windows 0.4 · 2026-09-20

4/4 CTest 测试组通过；正式接口与播放控制器的五档真实播放、原文/译文获取，以及真实 QML 搜索点击播放/歌词展示通过。详细范围、限制和日志见 [Windows 0.4 验证记录](docs/Windows-0.4-验证记录.md)。本轮不开发或验收 Android，不上传 GitHub。
...
深浅主题、分页布局和固定播放栏已完成 Windows 验证，版本号不变。四组回归测试通过，真实搜索/播放/歌词 UI 测试通过，2026-09-21 已启动打包程序供查看。详见 [UI 验证记录](docs/Windows-0.4-UI验证记录.md)。

## Android 0.4 同步补充 · 2026-09-21

手机主界面已同步，ARM64 APK 构建安装成功。Windows 四组回归测试通过，手机尺寸和桌面主题的最终布局检查通过。已实测真实搜索/音源/歌词、五档音源加载、暂停跳转、后台播放、悬浮页、连接失败及键盘返回。具体证据与范围见 [Android 验证记录](docs/Android-0.4-验证记录.md)。本轮不上传 GitHub。

## 0.4 提交前复查 · 2026-09-21

用户授权提交并上传当前 0.4。四组 CTest 重新运行，4/4 通过，耗时 16.40 秒；文档相对链接及 diff 空白检查通过。用户反馈的 Android 悬浮窗三项问题仅记录，未修改实现，见 BACKLOG.md 的 A04-01 至 A04-03；回归测试通过不代表这三项已修复。
